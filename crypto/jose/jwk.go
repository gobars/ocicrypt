...
func (key rawJSONWebKey) ecSm2PublicKey() (*sm2.PublicKey, error) {
	var curve elliptic.Curve
	switch key.Crv {
	case "S-256":
		curve = sm2.P256Sm2()
...
func fromRsaPrivateKey(rsa *rsa.PrivateKey) (*rawJSONWebKey, error) {
	if len(rsa.Primes) != 2 {
		return nil, gojose.ErrUnsupportedKeyType
	}
...
func (key rawJSONWebKey) ecSm2PrivateKey() (*sm2.PrivateKey, error) {
	var curve elliptic.Curve
	switch key.Crv {
	case "P-256":
		curve = sm2.P256Sm2()
	default:
		return nil, fmt.Errorf("go-jose/go-jose: unsupported sm2 curve '%s'", key.Crv)
	}
...
func tryJWKS(key interface{}, headers ...gojose.Header) interface{} {

...
type sm2EncrypterVerifier struct {
...
// newECDHRecipient creates recipientKeyInfo based on the given key.
func newSM2Recipient(keyAlg KeyAlgorithm, publicKey *sm2.PublicKey) (recipientKeyInfo, error) {
	// Verify that key management algorithm is supported by this encrypter
	switch keyAlg {
	case ECDH_SM2_S128KW:
	default:
		return recipientKeyInfo{}, ErrUnsupportedAlgorithm
	}
	...
	return recipientKeyInfo{
		keyAlg: keyAlg,
		keyEncrypter: &sm2EncrypterVerifier{
			publicKey: publicKey,
		},
	}, nil
}
...
func (ctx rsaDecrypterSigner) signPayload(payload []byte, alg SignatureAlgorithm) (Signature, error) {
	...
	case RS256, RS384, RS512:
		out, err = rsa.SignPKCS1v15(RandReader, ctx.privateKey, crypto.Hash(hash), hashed)
	case PS256, PS384, PS512:
		out, err = rsa.SignPSS(RandReader, ctx.privateKey, crypto.Hash(hash), hashed, &rsa.PSSOptions{
...
func (ctx rsaEncrypterVerifier) verifyPayload(payload []byte, signature []byte, alg SignatureAlgorithm) error {
	...
	switch alg {
	case RS256, RS384, RS512:
		return rsa.VerifyPKCS1v15(ctx.publicKey, crypto.Hash(hash), hashed, signature)
	case PS256, PS384, PS512:
		return rsa.VerifyPSS(ctx.publicKey, crypto.Hash(hash), hashed, signature, nil)
	}
...
// Encrypt the given payload and update the object.
func (ctx sm2EncrypterVerifier) encryptKey(cek []byte, alg KeyAlgorithm) (recipientInfo, error) {
	switch alg {
	case ECDH_SM2_S128KW:
	default:
		return recipientInfo{}, ErrUnsupportedAlgorithm
...
func (ctx sm2KeyGenerator) keySize() int {
	return ctx.size
}

// Get a content encryption key for ECDH-ES
func (ctx sm2KeyGenerator) genKey() ([]byte, rawHeader, error) {
	priv, err := sm2.GenerateKey(rand.Reader)
	if err != nil {
		return nil, rawHeader{}, err
	}

	out := cipher.DeriveSm2ECDHES(ctx.algID, []byte{}, []byte{}, priv, ctx.publicKey, ctx.size)
...
// Verify the given payload
func (ctx sm2EncrypterVerifier) verifyPayload(payload []byte, signature []byte, alg SignatureAlgorithm) error {
	var keySize int
	var hash x509.Hash
	...
	match := sm2.Verify(ctx.publicKey, hashed, r, s)
	if !match {
		return errors.New("go-jose/go-jose: ecdsa signature failed to verify")
	}
